...
import { isTestnet, shortAddress } from "../lib/network";
...
export function WalletDialog() {
  ...
  const [status, setStatus] = useState("Choose a wallet. Testnet is required.");
  ...
  async function connectFreighter() {
    ...
    if (!isTestnet(network)) throw new Error("Switch Freighter to Stellar Testnet.");
  ...
  async function connect(kind: "Freighter" | "Rabet") {
    ...
    try {
      ...
      setSession(next);
      setStatus(`${next.kind} connected to Testnet.`);
    ...
    <>
      ...
      <dialog className="wallet-modal" ref={dialog} aria-labelledby="wallet-title">
        <div className="modal-head">
          <div><span className="eyebrow">Stellar Testnet</span><h2 id="wallet-title">Connect a wallet</h2></div>
